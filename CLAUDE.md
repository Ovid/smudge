...
**Project sub-resources address the project by slug, except images and outtakes (F-24).** Six routers mount on `/api/projects` (`app.ts:41,45,46,50,52`). The decision on record is blanket — `docs/plans/2026-03-29-project-slugs-design.md:12`, "API uses slugs too. All project endpoints switch from `:id` to `:slug`", with exactly one carve-out (chapter endpoints stay UUID). Two later routers departed from it without recording why, and they are **not** the same case:
