...
**One task per session.** After completing each task (including its commit), stop and give the user a continuation prompt they can paste into a fresh session to pick up the next task. Do not start the next task in the same session unless the task cannot be implemented independently (e.g., it's a trivial config step that only makes sense bundled with the next task).

**Continuation prompt format:**

> Implement Task N of the plan at `docs/plans/2026-03-29-slice-0-1-plan.md`. The previous tasks (1 through N-1) are already committed. Read the plan, find Task N, and implement it following TDD red/green/refactor. Do not use a worktree — we're on the `ovid/mvp` branch.
