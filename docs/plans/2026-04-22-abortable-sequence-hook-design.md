...
- **Auto-abort on unmount.** The hook installs `useEffect(() => () => { bump(); }, [])` so every outstanding token becomes stale when the owning component unmounts. This closes the "forgot to bump on unmount" foot-gun — exactly the class of bug that motivated `useProjectEditor.test.ts:1417-1422`.
