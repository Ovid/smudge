...
| 5 | `search()` body: `searchAbortRef.current?.abort()` (early-bail prefix) | line 193 | Removed (the abort-prior is now done by `op.run()` itself). |
...
| `closePanel clears stale result state` | After `closePanel()`: capture the in-flight signal that was passed to the prior `mockFind` call; assert `signal.aborted === true`. |
...
### Three pure-new tests (added at end of file)
...
- All existing tests in `__tests__/useFindReplaceState.test.ts` continue to pass. Two existing tests gain `signal.aborted` assertions per §Test plan.
- Three new tests added to `__tests__/useFindReplaceState.test.ts`: abort-prior on rapid re-call, auto-abort on unmount, and success-path gate suppression.
