...
| 11 | Inline review-callout comments referencing `trashAbortRef`/`restoreAbortRef` by name | lines 31–35, 37–43, 75–79, 100–104, 111–115, 122–133, 159–165 | Pruned to remove stale ref-name references. **Preserve the *why* in every case**: zero-warnings invariant (the `aborted`-silent gates), `possiblyCommitted` UX rationale (lines 122–133), `seedConfirmedStatus` cache-seeding rationale (lines 100–104), shared-`trashAbortRef` cross-call-site sharing rationale (lines 159–165 → reframe as shared-`trashOp` rationale). The comments are the design-time enforcement of the invariants the tests pin; do not strip them, only retarget them. |
