...
| 18 | `components/SnapshotPanel.tsx` | ~148, ~172 (`fetchSnapshots` two sites), ~329 (`handleDelete`) | `snapshot.list` (148, 172); `snapshot.delete` (329) | Three sites, two scopes — one commit but verify all three. Lines 148 vs 172: distinguish the mount-effect call from the in-handler call by their surrounding context; both use `snapshot.list`. As-shipped tag: `(4b.3c.1 S18)` — the implementation commit (`2629dbe`) tagged this S18 instead of S15. Per /ovid-receive-code-review on 2026-05-26, retag was rejected (force-push cost > tag drift cost on a docs-only audit); plan table is the source of truth, this row is the only known overlap of S15 and S18 ids. | `refactor(snapshot-panel): migrate fetchSnapshots + handleDelete ladders to applyMappedError (4b.3c.1 S18)` |
...
- ✅ 16 simple-ladder migrations [S15] — Tasks 9-23 (note: Task 18's commit `2629dbe` shipped tagged `(4b.3c.1 S18)` instead of `(4b.3c.1 S15)`; documented in the Task 18 table row above. A grep-by-tag audit must consult the table, not the raw commit log, for this one row.)
