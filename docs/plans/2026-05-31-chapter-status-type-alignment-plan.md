...
- `src/__tests__/useTrashManager.test.ts` — fix the off-enum `"drafting"` fixture typo.
...
- [ ] **Step 8: Fix the latent off-enum fixture typo (finding [4])**

In `packages/client/src/__tests__/useTrashManager.test.ts` at line ~313, change:

```ts
    const restored = makeChapter({ id: "ch-restored", status: "drafting" });
```

to use a valid status that preserves the test's intent (a non-default status):

```ts
    const restored = makeChapter({ id: "ch-restored", status: "rough_draft" });
```

- [ ] **Step 9: Sweep for any other off-enum status literals**
...
Expected: no output after Step 8. Any remaining hit is another latent typo — fix it to the intended valid status (or, if it is not a chapter-status literal at all, e.g. an unrelated `status:` field, leave it). The authoritative completeness check is the next step.
...
- **Spec coverage:** schemas alias (T1), both shared fields (T2/3), write chain (T2/5), ref+seeder finding [1] (T2/4), DashboardView finding [2] (T2/6), exhaustive colors (T3/1), API payload+dashboard finding [3] + asymmetry finding [6] (T3/3), dead-branch note finding [3] (T2/6), RED-gate command finding [5] (execution notes + every typecheck step), fixture typo finding [4] (T2/8), grep sweep (T2/9), server cast (T2/7), CLAUDE.md deliverable (T4), `make all` (T5). All design sections map to a task.
- **Placeholder scan:** none — every code step shows the exact before/after.
- **Type consistency:** `ChapterStatusValue` spelled identically everywhere; ref type `Record<string, ChapterStatusValue | undefined>` consistent across `useProjectEditor.ts` and both `.types.ts` declarations; `status_summary` typed `Partial<Record<ChapterStatusValue, number>>` consistently.
