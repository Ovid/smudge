...
| ProjectSettingsDialog | prop | — (native focus) | false | **yes (NEW)** | **gains backdrop-dismiss** (see §Scope); slide-out inline-style CSS untouched; ✕ close button kept; field/timezone save logic untouched |
...
Leaning **yes**: add a short §Key Architecture Decisions note that dialog
lifecycle (show/close sync, focus-on-open, Escape, backdrop) routes through
`useDialogLifecycle`, so future dialogs adopt one policy rather than copying a
neighbour — parallel to the existing `useEditorMutation` /
`useAbortableSequence` / `useInlineTitleEditing` entries. Folded into the plan
as an explicit deliverable. Final wording decided at step 7.
