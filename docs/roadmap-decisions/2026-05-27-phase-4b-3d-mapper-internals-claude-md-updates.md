...
- **Post-execution amendment (2026-05-28, review round 2 [I1]):** the branch as originally pushed shipped an imperative-refresh-vs-mount-fetch test ("imperative refreshSnapshots() invalidates a concurrent in-flight mount fetch") in place of the chapter-switch test the resolution above promised. The two tests ride the same hoisted `chapterSeq.abort()` mechanism but pin observably distinct contracts — chapter-switch (the design's load-bearing case) and same-chapter imperative refresh (the design's "harmless / arguably more correct" case). Round 2 review surfaced the divergence. Resolution: the chapter-switch test the design promised was added in a follow-up commit (verified meaningful by temporarily reverting the hoist and confirming RED before restoring), and the imperative test was kept since it covers a real adjacent scenario. Both now pin the post-hoist semantics; design and plan amended to describe both.
...
## Round 2 Review (2026-05-28)

A multi-agent agentic-review pass on commit `c8c8b48` surfaced 5 in-scope findings and 2 out-of-scope items (paad/code-reviews/mapper-internals-claude-md-updates-2026-05-28-09-52-31-c8c8b48.md). Resolutions:

- **[I1] [S14] test diverged from design** — addressed by adding the chapter-switch test the design promised, keeping the imperative test, and amending design.md + plan.md + pushback issue [3] above to describe both. Verified meaningful by temporary revert of the hoist.
- **[I2] refreshTrashList mock never invoked the factory** — addressed by rewriting the mock as a passthrough that exercises the real factory, adding `vi.mock("../api/client")`, and asserting `trashOp.run` was called and `api.projects.trash` received the project's slug + captured signal. The structural-check comment that appealed to this guarantee now matches reality.
- **[S1] `KNOWN_DELEGATION_HELPERS` regex limitation** — addressed by documenting the `[^)]*`-cannot-span-nested-parens constraint in the comment block at `migrationStructuralCheck.test.ts:118`, symmetric with the existing `[^>]*` non-nested-generic note.
- **[OOSS1] `SnapshotPanelHandle.refreshSnapshots` type drift** — tightened from `() => void` to `() => Promise<void>`; EditorPage's six fire-and-forget callers continue to typecheck via implicit Promise discard, and the new chapter-switch test's `await` now has a correct declared shape.
- **[OOSA1] structural-check additions not enumerated by spec** — acknowledged as a load-bearing prerequisite for [S13]'s green-gate (without `KNOWN_DELEGATION_HELPERS`, the structural check would have flagged `trashOp` as a dead binding). Plan amended (`Post-execution amendments` in Self-Review Notes) to acknowledge the planning gap and document the work as part of [S13]'s scope rather than scope creep.
