...
## Code-Review Follow-ups (2026-07-11, HEAD a1e0ba5)

A second agentic review (`paad/code-reviews/operational-backup-stopgap-2026-07-11-14-00-01-a1e0ba5.md`) surfaced 2 Important + several Suggestion findings, all fixed on this branch: I1/F3 (ENOENT-tolerant image read during live backup), I2/F6 (restore reuses shared `parsePort`/`DEFAULT_SERVER_PORT`), S-F4 (non-truncating flag parse), S-F5 (rotateAutoBackups re-throws non-ENOENT), S-F2 (partition-aware free-space check for external `DB_PATH`), S-F1 (UTC-sortable backup filename stamp so rotation survives a DST fall-back), and F7/S-F8/S-F10 (doc/comment reconciliations). F7's lying-CD RAM-bomb bypass remains accepted as backlog S1 (design doc reconciled, no mitigation shipped).

### One-feature-rule exception: S-F9 (`getImagesDir()` refactor)
- **Decision:** APPROVED by the user (Ovid) on 2026-07-11 to include the S-F9 cross-cutting refactor on this feature branch rather than deferring it to its own PR.
- **What:** `config/paths.ts` gains `getImagesDir(dataDir?)` — the single owner of the `images` subdir name — and the four previously-hardcoded `join(dataDir, "images")` sites (`images.paths`, `images.reaper`, `db/purge`, `backup-core`) route through it. This extends the F-5 "single owner of data locations" invariant that `config/paths.ts` already documents.
- **Why the exception is justified:** the duplication was flagged *because this phase added the fourth copy* (`backup-core`), so the dedup is directly continuous with the phase's own work; the change is small, mechanical, behavior-preserving (existing tests for all four sites are the regression guard), and touches no feature logic. The alternative — routing only `backup-core` — would ship a `getImagesDir()` that three of four sites ignore, which is strictly worse. The one-feature rule "defaults to enforcement"; this is a recorded, deliberate exception per that rule's escape hatch.
