...
### Phase Numbering and Order

**Phase numbers are stable identifiers, not execution order.** Items are handled in the order they appear in this document — top to bottom — and that document order is the source of truth for "what's next." Phase numbers exist to give each phase a stable name that survives reordering: when a phase is moved, its number stays the same so external references (commits, decision-log entries, code-review artifacts, conversations) keep working. Reading "Phase 4b.3a.1" sandwiched between "Phase 4b.3a" and "Phase 4b.3b" is correct — that's a phase whose number reflects its current execution-order position because it was renumbered when moved. The number `4b.3a.1` does not imply anything about when it was created or how it relates to `4b.14` historically; it just identifies a unique row in the table below.

**Implication for tooling.** Any tool that picks "the next phase to plan" must scan this document in document order, not numeric order. The `/roadmap` skill follows this contract: it walks the file top to bottom and picks the first `## Phase` heading without a `<!-- plan: … -->` comment. Future tooling should follow the same rule.
...
- [S12] `api.chapters.get` signal threading at remaining `useProjectEditor.ts` call sites (lines 245, 704, 757). The fourth call site (line 849) already threads a signal. These are paired with `selectChapterSeq` (a `useAbortableSequence`) for epoch checks; the `AbortController` complement is orthogonal. Likely keeps a hand-rolled `selectChapterAbortRef` pattern, since Phase 4b.3a.1's "Out of Scope" explicitly notes that the `useAbortableSequence`-paired sites stay paired.
...
<!-- plan: 2026-05-27-mapper-internals-claude-md-updates-design.md -->
...
<!-- plan: 2026-05-29-editor-state-machine-design.md -->
...
<!-- plan: 2026-05-31-tiptap-extension-consolidation-design.md -->
...
<!-- plan: 2026-05-31-chapter-status-type-alignment-design.md -->
...
<!-- plan: 2026-05-31-shared-tiptap-unsafe-keys-set-design.md -->
...
<!-- plan: 2026-06-01-inline-title-editing-hook-design.md -->
...
<!-- plan: 2026-06-03-dialog-lifecycle-hook-design.md -->

### Goal

Extract `useDialogLifecycle({ open, onClose, initialFocusRef, blockEscapePropagation }) => { dialogRef, onBackdropClick }` capturing the show-on-mount, focus-an-actionable-element, Escape-closes (sometimes with `stopImmediatePropagation`), backdrop-click-closes pattern that is reimplemented across `ConfirmDialog`, `ExportDialog`, `NewProjectDialog`, `ProjectSettingsDialog`, and `ShortcutHelpDialog`. The hook creates and returns the `dialogRef` (callers no longer pass one in). Migrate the dialogs one at a time behind characterization tests; preserve `stopImmediatePropagation` as an opt-in, make the `showModal/close` try/catch an always-on guard, and keep ARIA `role` in each component's JSX.
...
- Add `useDialogLifecycle` in `packages/client/src/hooks/` with unit tests. The options expose `open`, `onClose`, an optional `initialFocusRef` (focus a specific element after `showModal()`; `ConfirmDialog` and `ExportDialog` opt in), and `blockEscapePropagation: boolean` (default false; `ConfirmDialog` opts in). The `dialog.showModal()` / `dialog.close()` calls are wrapped in an always-on try/catch (no caller needs the throw to surface, so the originally-considered `safeShowClose` opt-in was dropped). ARIA `role` is **not** a hook option — it stays in each component's JSX. The hook creates the `dialogRef` and returns it alongside an opt-in `onBackdropClick` handler.
...
<!-- plan: 2026-07-11-abortcontroller-eslint-rule-design.md -->
...
- Add `packages/client/src/hooks/usePersistedState.ts`: `usePersistedState<T>(key,
...
**Versioning is forward-only.** A v4-format bundle imports into v7 Smudge by running Knex migrations 5, 6, 7 against the embedded SQLite. A v7-format bundle does _not_ import into v4 Smudge — it's refused with a clear "this backup was made by Smudge vN, you're running vM; upgrade to open" message. Single-user app; the user controls what version is installed.
...
- The Phase 4b.14 operational backup stopgap is _not_ a dependency — it's a sibling that bridges the gap until 8b ships and may live alongside 8b post-ship as a "full-machine snapshot" complement to per-project `.smg`.
