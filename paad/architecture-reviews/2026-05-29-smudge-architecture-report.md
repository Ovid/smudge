...
- **Found by:** Error Handling & Observability
- **Status:** Fixed
- **Status reason:** Added a lightweight request-correlation middleware `packages/server/src/requestContext.ts`, mounted in `createApp()` **before** `express.json` (so even a malformed-body 400 is traceable). It (a) honours a well-formed inbound `X-Request-Id` (bounded `^[A-Za-z0-9._-]{1,128}$` allowlist — rejects control chars / log injection / unbounded data) or mints a `randomUUID()`, (b) exposes it as `req.id`, (c) attaches `req.log = logger.child({ req_id, method, path })`, (d) echoes the id in the `X-Request-Id` response header (so a client/proxy can correlate), and (e) on response `finish` emits a `debug`-level `"request completed"` access log — silent at the default `info` level (no test/prod noise; available via `LOG_LEVEL=debug`). The global error handler now logs unhandled errors with `{ err, status, req_id, method, path }`, so a 500 in the logs ties back to its request — closing the exact gap the flaw names ("no way to correlate it to the request"). **Design choice (test-safe + spy-stable):** the error log stays on the **top-level** `logger` (not `req.log`) so the single `"Unhandled request error"` call site is preserved and every existing `logger.error` spy/suppression across the suite keeps working; correlation is carried as explicit fields instead. `req.id` is read defensively in the handler in case an error escapes before the middleware ran (e.g. the bare-app error-handler unit test, where it is simply `undefined`). **Scope honesty:** this delivers request-id correlation + a debug access-log hook — the lightweight option the report's Next-Question 5 floats. It does **not** add a metrics/Prometheus surface (a larger dependency decision, out of scope for a single-user app today), and it does not thread `req.log` into the service layer (services still log via the module `logger`; the child logger is exposed for future handlers that hold `req`). **Red→green safety net:** new `request-context.test.ts` (4 cases) pins header generation, valid-id propagation, invalid-id rejection→fresh-UUID, and the error-log correlation fields (`req_id`/`method`/`path`); the existing 8 `error-handler.test.ts` cases stay green (bare app → `req.id` undefined → top-level `logger.error` still spied). Full server suite green (659 tests, +4); the `debug` access log produced zero added output at the default level. Typecheck + `lint:check` (`--max-warnings 0`) clean.
- **Status date:** 2026-05-29
- **Status commit:** 54548bf70cec6ae38bd346ab9c4303f3d9c94097
...
- **Found by:** Structure & Boundaries
- **Status:** Fixed
- **Status reason:** Extracted the raw filesystem calls (`mkdir`/`writeFile`/`readFile`/`unlink`) out of `images.service` into a dedicated, named seam `packages/server/src/images/images.fs.ts` (`writeImageFile` / `readImageFile` / `deleteImageFile`). `images.service` no longer imports `node:fs/promises` — image binary I/O now has a single named boundary that can be mocked in one place, so the service's *logic* (validation, refcount transaction, orphan-cleanup ordering) is unit-testable without touching the disk. This directly addresses the flaw ("the one service whose I/O has no injectable/mockable seam"). **Deliberately NOT the removed `AssetStore`:** the report notes a full `AssetStore` DI interface "was added then removed as unused" — re-introducing speculative constructor-injected DI for a single production filesystem backend would re-add the over-abstraction that was correctly deleted. A module seam (plain functions, imported directly) gives the mockability the flaw asks for **without** resurrecting that abstraction or changing the service's signature/DI shape. The blob-vs-rows asymmetry the report calls "defensible" is preserved by design — images still don't go through `ProjectStore`; they now go through a clearly-named fs module instead of inline `fs` calls. **Behavior byte-preserved** (pure extraction): the 23 real-filesystem `images.service.test.ts` cases (upload→writeFile, serve→readFile, delete→unlink, insert-failure→orphan-unlink, unlink-fails-gracefully) exercise the seam end-to-end against a temp dir and all stay green (81 image+export-image tests pass). The shared `deleteImageFile` is reused by the F-14 orphan reaper. Typecheck + `lint:check` (`--max-warnings 0`) clean.
- **Status date:** 2026-05-29
- **Status commit:** e98d20e2e182dbb131018ec6ccb6b03542b8395f
...
- **Found by:** Integration & Data
- **Status:** Fixed
- **Status reason:** The flaw correctly notes the FS ops **cannot** join the SQLite transaction and that the "orphan file (harmless)" outcome is the right trade-off — so the design (write-then-insert with insert-failure unlink; commit-then-unlink on delete) is **kept**. What it also named as the real gap is closed: "unlike soft-deleted rows there is no startup reaper for image files." Added `packages/server/src/images/images.reaper.ts` → `reapOrphanImages(db, dataDir?)`, wired into `index.ts` startup right after `purgeOldTrash` (the established startup-reaper pattern). It loads the set of known image ids, walks `DATA_DIR/images/<project>/`, and deletes only **image-shaped** files (`<uuid>.<ext>`, matched against the shared `UUID_PATTERN`) whose id has no DB row — so a crash between an upload's failed `INSERT` (file already written) or between a delete's commit and its `unlink` no longer strands a blob forever. **Safety guarantees:** (1) runs once at startup when no upload is in flight, so it cannot race a mid-upload file whose row has not yet committed (the same reason `purgeOldTrash` is startup-only); (2) only ever deletes files whose name matches the `<uuid>.<ext>` shape this app writes — any other file on disk (e.g. `notes.txt`) is left untouched; (3) best-effort per file — failures are logged via the structured logger and skipped, never thrown. Reuses F-13's `deleteImageFile` seam. **Red→green safety net:** new `images.reaper.test.ts` (3 cases) pins orphan-deleted-but-known-kept, non-image-files-never-touched, and missing-images-dir→0. Full server suite green (662 tests, +3); typecheck + `lint:check` (`--max-warnings 0`) clean. **Scope honesty:** this is a *reaper* (eventual reclamation on restart), not transactional FS — making image writes truly atomic with the DB is impossible without a 2-phase protocol the report itself rules out; the orphan window between commit and unlink still exists momentarily, it is just no longer permanent.
- **Status date:** 2026-05-29
- **Status commit:** c47e5e8f1c8b1b3c1d1f4bed26d34b7cc8b3c11a
...
- **Found by:** Structure & Boundaries
- **Status:** Fixed
- **Status reason:** Removed all three module-scope mutables — `nextEditorId`, the `imageUploadHandlers` Map, and `activeEditorId` (plus the per-component `editorIdRef` and the `onFocus` handler that maintained `activeEditorId`). The paste/drop upload handler is now **instance-scoped**: the `imagePaste` extension takes a `getUploadHandler` option, its ProseMirror plugin calls `getUploadHandler()` at paste/drop time, and each `<Editor>` configures the extension with a stable getter (`useCallback`) over its own `uploadHandlerRef`. The registration effect sets `uploadHandlerRef.current` (and clears it on unmount) instead of writing the module Map. Because each editor's plugin closes over its own getter, the design is now correct if two editors are ever mounted simultaneously — the latent hazard the flaw names — with no shared "most-recently-focused" global. **Why a getter-over-ref (not `editor.storage` or a raw ref option):** the project's strict `react-hooks` rules reject mutating a hook return value (`editor.storage` → `immutability`) and passing a ref object into a render-time call (`refs`); a getter invoked only at event time is the canonical bridge into the imperative editor. The one residual `react-hooks/refs` report on the `configure({ getUploadHandler })` line is a conservative false positive (the getter is never called during render — only inside the ProseMirror plugin at event time), suppressed with a single justified `// eslint-disable-next-line react-hooks/refs -- …` carrying that reason (the repo's established pattern for documented false positives). **Behavior preserved with one defensive guard (S8 correction):** the in-flight-upload guards (project-id capture `uploadProjectId`, instance-identity `startEditorInstance`, `editor.isDestroyed`) and the I8/I9/S18 review fixes are unchanged — only the handler *plumbing* moved off the module globals. The rewritten registration effect does add an `if (!editor) return;` early-return that the pre-fix code did not have (the prior `imageUploadHandlers.set(id, …)` ran unconditionally). Functionally equivalent — under the pre-fix code the handler closure's own `if (editor && !editor.isDestroyed)` guard already covered the null-editor window, and the plugin can only fire on a mounted editor anyway — but the explicit early-return now avoids ever installing a closure over a null editor on the new instance-scoped ref, so the original "byte-preserved" phrasing was an overstatement. **Safety net:** the existing `Editor.test.tsx` paste/drop suite (handlePaste/handleDrop happy-paths, the 2xx-BAD_JSON committed/announce cases I3/I8, and the "no image files → false" negatives — 36 tests) exercises the plugin via the real editor instance and all stay green; full client suite green (1436 tests). Typecheck + `lint:check` (`--max-warnings 0`) clean.
- **Status date:** 2026-05-29
- **Status commit:** ed7daf98feb63b82cf3d4f6a72802120d5642a38
