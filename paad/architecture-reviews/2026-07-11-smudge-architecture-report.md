...
- **Status reason:** Structure accepted as a deliberate trade-off (recorded in CLAUDE.md §Accepted Architectural Trade-offs); the actionable data-loss risk mitigated by a new enforcement test. Rationale: five prior decompositions already extracted rendering + the controller hooks, so the residual concentration is irreducible cross-hook coordination, not accidental complexity. The guard "duplication" is essential — each site carries a distinct dated review annotation (I1/I2/I4) and a per-site return/banner contract (`switchToView`'s locked branch deliberately shows no banner), so collapsing it into one helper would destroy the audit trail. The only structural "fix" (a context/provider lift) was rejected: it would hide invariant-critical shared mutable state and buy near-zero safety, since the two controllers already receive that state via compile-checked typed deps (`FindReplaceControllerDeps`/`SnapshotControllerDeps`) — mis-wiring is already a compile error. The one real residual — no mechanical enforcement that a NEW editor-mutating entry point is guarded — is closed by `packages/client/src/__tests__/editorEntryPointSurface.test.ts`: a forcing-pause snapshot of the full entry-point surface across all four EditorPage wiring sites (`EditorHeader`/`EditorMainContent`/`EditorDialogs` props + `useKeyboardShortcuts` keys) that goes red when any entry point is added/removed, forcing the author to choose its guard axis per CLAUDE.md save-pipeline invariants. Proven red against a real injected entry point (`onFrobnicate`) and reverted; 8 tests incl. drift-detector self-tests; full suite 2649 pass, coverage gate held. Honest ceiling: converts reviewer-optional → author-mandatory acknowledgment; it does not verify guard *correctness* (the behavioral tests in `EditorPageFeatures.test.tsx:1402/1455` do that for current handlers). Decision reached after two adversarial `/pushback` passes rejected (a) the context/provider refactor and (b) an `isActionBusy`-scan regression-lock that audited the concurrency axis rather than the lock axis the data-loss actually rides on. Ovid's decision (2026-07-11).
- **Status date:** 2026-07-11 19:28 UTC
- **Status commit:** dbe9b6a
