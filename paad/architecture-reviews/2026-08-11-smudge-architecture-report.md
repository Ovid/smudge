...
- **Impact:** High
- **Explanation:** The full resolved import graph — including `import type`, `export … from`, dynamic `import()`, and `@smudge/shared` subpath exports — is a DAG. The verifier rebuilt the graph independently over a _superset_ of files (180 vs the specialist's 161, adding `scripts/` and `e2e/`) and found 0 unresolved specifiers and 0 cycles, counting type-only edges.
...
- **Impact:** Medium _(downgraded from High: in a three-package monorepo where `shared` is by construction the leaf, this is closer to structural than hard-won)_
...
- **Evidence:** `grep -rn "@smudge" packages/shared/src` (non-test) → 7 hits, all comments. `packages/client/src/sanitizer.ts:78,103` and `packages/server/src/export/export.renderers.ts:36,58,72` are comments, not imports. Five+ shared parity tests read sibling sources as _text_ via `readFileSync` — not module edges.
- **Found by:** Coupling & Dependencies
...
- **Impact:** Medium
- **Explanation:** The ZIP byte-format/security layer was pulled out of backup lifecycle orchestration into a dependency-free module, and the header argues why a _module_ rather than a region.
...
- **Category:** S3 (Loose coupling)
- **Impact:** Low _(downgraded from Medium: three imports total)_
- **Explanation:** Components import hooks freely; hooks never import a component _value_. The three hook→component edges that exist are all `import type`, closing the one direction that would create a runtime cycle.
...
- **Impact:** Medium
- **Explanation:** A corrupt row is substituted with a doc that _passes_ `TipTapDocSchema`, so "empty" cannot be read as "safe"; the flag is optional-and-omitted on the happy path (never a stale `false`), and every consumer tests the flag rather than emptiness.
...
- **Impact:** High
- **Explanation:** Five independent bounds on one endpoint, each guarding a distinct amplification route, all throwing _inside_ the transaction so a partial replace never persists; neighbouring TipTap walkers fail closed rather than guessing.
- **Evidence:** `packages/server/src/search/search.service.ts:33` (`REGEX_DEADLINE_MS = 2_000`, checked at `:139` and `:255`), `:66-71` (compiles with the same flags runtime uses so a `\p{L}` pattern 400s instead of 500-ing later), `:284` (`max_output_chars` applied _during_ per-match expansion "so pathological `$'` amplification can't allocate gigabytes of intermediate strings"), `:319-321` (`Buffer.byteLength` vs `MAX_CHAPTER_CONTENT_BYTES`), `:346-360` (all four typed throws → 400). Fail-closed neighbours: `images.references.ts:141-147` ("aborting diff to avoid mass decrement"), `:207-222` (`scanChapterContentForImage` returns `"unreadable"`, blocking an irreversible delete).
- **Found by:** Integration & Data
...
- **Impact:** Medium
- **Explanation:** A `TypeError` during a body read means the stream broke post-headers (the server never flushed a body), so it routes to NETWORK/transient rather than BAD_JSON — which would otherwise raise a _false_ "possibly committed" lock banner.
...
- **Category:** S8 (Observability present)
- **Impact:** Low
- **Explanation:** When canonicalization fails, the warn fires once per _unique_ corrupt content (sha256-keyed) and drops to `debug` on repeats, so a corrupt chapter row hit repeatedly cannot bury unrelated warnings.
...
- **Impact:** Medium
- **Explanation:** All four persistence locations derive from one module; `DB_PATH` defaults _through_ `DATA_DIR` so the two cannot silently point at unrelated directories, and `getImagesDir(dataDir?)` takes an explicit override precisely so backup/restore/purge/reaper thread their own dir instead of reading env.
...
- **Category:** S9 (Configuration discipline)
- **Impact:** Medium
- **Explanation:** Where two representations of one limit used to sit side by side with a comment insisting they agree, the second was _deleted_ and derived instead; the unit a cap is measured in is stated because a grapheme-vs-code-unit mismatch already produced a real bug.
...
- **Exception named to keep the claim accurate:** "exactly one `generateHTML` call site" is true for _production_ only. Two shared test files call it directly (`editorExtensions.test.ts:81`, `noteMark.test.ts:26,33,42,49`) — deliberately, since `noteMark.test.ts:26` is the test proving an unstripped doc _does_ leak.

### [S-22] One fail-closed image-src rule encoded three times, held together by a test that reads the regexes out of source

- **Category:** S10 (Security built-in)
- **Impact:** High
- **Explanation:** Where the accepted F-16 trade-off forbids unifying the client and server URI rules, the codebase substituted a mechanical parity check: a shared test extracts all three regex literals _from their source files_ and runs one corpus through all three, so drift turns red instead of silently deleting images from exports.
...
- **Found by:** Security & Code Quality
- **Verification:** The verifier _ran_ the four suites read-only — 44 tests, 4 files, all passing. This is enforcement, not documentation.
...
- **Category:** S10 (Security built-in)
- **Impact:** Low _(downgraded from Medium: absence-of-secrets is table stakes; the coherent posture is the part that carries weight)_
...
- **Status caveat:** the residual uncovered lines are the `_exhaustive: never` guard (374-375), which is a compile-time exhaustiveness check and unreachable at runtime by construction. The global thresholds in `vitest.config.ts` still cannot see a single file's coverage — this fix closes the hole but does not close the _class_ of hole the finding's last sentence identifies.
...
- **Impact:** Medium _(down from Medium-High)_
...
- **Found by:** Structure & Boundaries
- **Why this is not a restatement of accepted F-19:** F-19's premise is that `updateChapter`/`deleteChapter`'s side effects are _enumerated in the function's doc comment_. That discipline lives only on `chapters.service.ts`; `restoreSnapshot` and `replaceInProject` carry no such enumeration and are not mentioned by F-19. The claim here is about the sequence being triplicated, not about it being undocumented in one place.

### [F-04] The outtakes feature cluster landed inline in `EditorPage` instead of the controller-hook pattern

- **Category:** 13 (Inconsistent boundaries) _(re-typed from 2/3)_
- **Impact:** Medium
...
- **Found by:** Structure & Boundaries, Coupling & Dependencies (independent agreement)
- **The accepted-F-1 premise genuinely changed.** F-1's load-bearing sentence is _"the residual concentration is irreducible cross-hook coordination, not accidental complexity."_ The capture flow's own comment refutes that for itself — `EditorPage.tsx:1023-1025`: _"It never writes editor content, so save-pipeline invariants 1-4 do NOT apply and NO busy/lock guard is needed."_ A block that explicitly participates in none of the cross-hook coordination is, by F-1's own definition, not the irreducible residual. Partially blunted: `handleInsertOuttake` sits next to `handleInsertImage` and deliberately shares `guardInsertAtCursor` (`:555-589`), so extracting it would split a deliberately-paired guard. The capture handler is the part with no such tie.
- **Status:** Fixed
...
- **Status correction (the finding overstates its own line-count clause):** F-04's Explanation closes with _"taking `EditorPage.tsx` to 1,356 lines"_, but its argument (and the reason this fix is justified) is the coordination premise it borrows from F-1, not size. F-1 already accepts the file being "the largest file in the client" — conditionally, on the residual being irreducible coordination — and never names a number. Size is a symptom here; the fix was scoped to the block that provably participates in no cross-hook coordination, which is what F-04 actually argues.
- **Status note (the destructive sibling will not inherit the exemption):** roadmap Phase 4c.2a ("cut selection to outtakes", destructive) is planned and the roadmap states it _"touches the save-pipeline invariants and gets its own PR"_. Capture is exempt from invariants 1-4 only because it never writes editor content; the cut does. The hook is therefore named for the one flow it owns rather than for outtakes generally, and its header records why 4c.2a cannot simply be added alongside this handler.
- **Status caveat (what the safety net does and does not pin):** the 19 behavioural cases in `OuttakesEditorEntryPoints.test.tsx` drive a real `EditorPage` mount and passed unmodified across the move — no test was edited. One was added first (commit `4c189ada`) because coverage showed the success arm's `signal.aborted || isStaleProject()` guard partial, and it asserts on the announcement rather than the prepended row: the panel independently refuses a foreign-project row, so a row-based assertion stays green with the guard deleted. Confirmed red against that deletion. `editorEntryPointSurface.test.ts` does **not** guard this refactor — it matches prop _names_, which are unchanged; it stayed green and needed no edit.
...
- **Status note (why not the delete-side scan):** blocking the delete was the recommended option until `/paad:rethink` verified its premises. Three findings moved it: (1) the claim that it was "the smaller, safer change" was **false** — it needs the snapshot scan _plus_ a new error-payload key, new scope copy, a new client branch, and an extension of the separate `/references` endpoint that powers the pre-delete confirm gate, which is otherwise snapshot-blind and would leave an enabled Delete button that 409s by surprise; (2) the 409's existing channel is announce-only and its copy is hard-coded _"Remove it from those chapters first"_ — advice a user **cannot follow** for a snapshot; (3) snapshots have **no retention of any kind** (verified whole-repo: no `.limit(`, no scheduler, no reaper touching the table; the only automatic removal is the FK cascade when a chapter row is hard-purged 30 days after trashing), so blocking would pin an image permanently and the only remedy would be deleting history to delete an image.
- **Status note (why degrading is safe here):** the image bytes are already unrecoverable at this point — images have no `deleted_at` in any migration and the blob is unlinked immediately (both verified). Refusing the restore therefore cannot bring the image back; it only _also_ withholds the prose. `GET /api/snapshots/:id` was verified to still return 200 with full content after the delete, so the words were provably sitting there behind a refusal. The count is announced on **both** success arms, including the stale-chapter switch — a user who navigated away is no less entitled to know their content was altered.
- **Status caveat:** this is the only path in the codebase where a restore returns something other than what was saved. That is a deliberate exception to the "restore is byte-exact" posture, justified only because the alternative is a permanent dead end, and it is guarded by the mandatory announcement. It does **not** license further content mutation on restore. Separately, F-05's delete-side asymmetry is now _intentional_ rather than fixed: `deleteImage` still ignores snapshots, so a delete can still strand images inside snapshots — the difference is that the snapshot now survives it.
...
- **Impact:** Medium _(down from High)_
...
- **Evidence drift (2026-08-17, from the F-08 fix — this finding is still open and unfixed):** the text above describes the machine as left at `{editable:false, busy:true}`. There is no longer a `busy` field (F-08 removed it as an unread mirror), so the stranded state is `{editable:false}` with no lock. The _substance_ of F-07 is unchanged — the `finally` block still dispatches nothing when `reloadFailed` is set, and the consumer still owns `COMMITTED_UNRELOADED` — but a future session should not go looking for `busy`. F-08's fix also settled a question relevant here: a seam-level fix would **not** want `busy` back, because the re-entrancy latch must be readable before the first `await` and reducer state is not.

- **Status:** Partially fixed — the seam is unchanged; a detection test now blocks a silently-added third caller
- **Status reason:** Closed with a forcing-pause test rather than a change to the seam: `packages/client/src/__tests__/mutationCommittedSurface.test.ts` discovers every `mutation.run()` caller under `packages/client/src` and pairs each file's call count against its `committed_but_unreloaded` branch count. A third caller landing without a handler turns the surface assertion red; an existing caller losing a handler turns the pairing assertion red. Both were verified by negative control against the real source, and each fired on the _correct_ assertion (a synthetic caller added to `EditorPage.tsx` → surface; one of `useFindReplaceController`'s two branches renamed → pairing, "expected 1 to be 2"). Comments are excluded, so the `useEditorMutation.run()` spelling used in prose throughout the editor hooks does not create phantom callers; parser self-tests pin that, on the `editorEntryPointSurface.test.ts` precedent, so a silently-matching-nothing regex cannot make the check vacuous.
- **Status correction (2026-08-19 code review, I1/I2 — the guard as first shipped did not hold in the direction claimed above):** caller discovery was `/\bmutation\.run\s*[<(]/`, keyed on the literal receiver name, so a third caller naming its handle anything but `mutation` (`editorMutation`, `snapshotMutation`, `mutation?.run(`, a Prettier wrap before `.run`) added no key and the surface assertion stayed **green** — the one direction that had to fail loud was the one that failed silent. Separately, the comment filter tested only whether a line _started_ with a comment token while matching against the whole line, so a trailing `// stage === "committed_but_unreloaded"` counted as a handler branch. Both are fixed: discovery now extracts the useEditorMutation **handle binding** (which TypeScript forces to spell the hook's name, whether the file constructs it or declares a field typed `ReturnType<typeof useEditorMutation>` / `UseEditorMutationReturn`) and counting runs over comment-stripped source via the sibling detector's `stripCommentsFromTsSource`. `pages/EditorPage.tsx` now appears in the committed surface at count 0 — it owns the shared instance and calls `run()` nowhere — so the day it starts calling `run()` the assertion goes red **for the `<handle>.run(` spelling** — see the round-3 correction below for the two spellings that stayed green. The claim above holds only as narrowed there. Fix commits: `36169a38` (I2), `46ff26fb` (I1).
- **Status reason (why not fix the seam):** Options were weighed explicitly with the developer. Moving the terminal dispatch into `run()` — making the seam decide on-target-vs-drifted and dispatch `COMMITTED_UNRELOADED` or `MUTATION_SETTLED_SUPERSEDED` itself — is the only option that makes the failure _structurally_ impossible, and both consumers do now implement the same two-case split by hand, so the duplicate is real rather than speculative. It was rejected for this session on cost: it reshapes the save pipeline (CLAUDE.md records 16 review rounds for the last such reshaping), changes the call shape of ~50 tests, and collides with a frozen safety-net assertion (`useEditorMutation.test.tsx` pins the committed path's terminal event list at exactly `["MUTATION_STARTED"]`, which is precisely the behaviour that option changes). That option remains available and is the right one if a third consumer ever lands. **The trigger is wired (S10, review round 3):** the `*** STOP` block in `mutationCommittedSurface.test.ts` — the text the third caller's author reads at exactly that moment — now points here, so the decision reaches the person the decision is about.
- **Status correction (the finding's own evidence overstates its case):** F-07 says this "has already produced a live defect", citing OOSI1. Reading that fix (`useFindReplaceController.ts`, the `reloadFailed && stale` branch) shows OOSI1 was **not** a caller ignoring `committed_but_unreloaded` — the caller handled the stage. It handled one sub-case (user still on the replaced chapter) and omitted the other (user had drifted away), setting a dismissible error without re-asserting editability. That is a _partial-handling_ bug, not a _missing-handling_ bug. **This test would have been green throughout OOSI1**, and so would a compile-time obligation to supply a handler. Only the seam-owns-the-dispatch option would have prevented it. The distinction is recorded here because the finding's phrasing invites a future session to over-credit this fix.
- **Status caveat:** this is author-mandatory acknowledgment, not verification — the same limit CLAUDE.md already states for the F-1 analogue ("it does not verify the guard is _correct_"). Correctness of the current handling stays with the behavioural tests in `useFindReplaceController.test.tsx` and `useSnapshotController.test.tsx`. The pairing is per-file rather than per-call, so a future refactor that centralises handling across a file's two callers will fail loudly and demand a decision rather than passing silently — the safe direction, and noted in the test header.
- **Status correction (2026-08-19 code review round 2, I1/I2 — the guard still did not hold in the direction claimed above):** round 1 keyed discovery on the useEditorMutation **handle binding** and asserted that TypeScript forces the hook's name to be spelled out wherever a handle is obtained. It does not. Two reachable shapes bind no name — a one-step `const { run } = useEditorMutation({...})` destructure, and a consumer whose deps interface was hoisted into a sibling `*.types.ts` (`useProjectEditor.types.ts` is the local precedent) — so a new caller in either shape still added no key and the surface assertion still passed **green**. Separately, the counting pass saw only `<handle>.run(`, and the file claimed any other spelling "fails toward red": true when an unseen shape _replaces_ a counted call, false when one is _added_, which is the "third caller lands" case the guard exists for. Both reproduced against the real `useFindReplaceController.ts` using the shipped helper bodies. Fixed: discovery now lists a file on any of three signals (binds a handle / calls `run()` on a handle name bound anywhere in the tree / imports the hook), and `findUncountableRunShapes` refuses the unattributable call shapes outright rather than relying on the numbers to notice. **Ceiling (round 3):** as shipped in this round the refusal was neither handle-independent nor tree-wide, so two shapes still passed green — see the round-3 correction below. Both verified by negative control — probe files in each shape turn the surface assertion red, and an appended destructured `run()` turns the new refusal red.
- **Status caveat (the ceiling is now stated, not asserted away):** the header used to claim there was exactly one remaining unseen shape. It now names the three discovery signals and says plainly that a spelling covered by neither the counter nor the refusal will not be noticed by the numbers — extend one of them rather than assume.
- **Status correction (2026-08-19 code review round 3, I1/I2 — the third consecutive round to find the guard green-passing a shape it was recorded as blocking):** round 2 shipped two fixes that disagreed about which handle set to scan, so their **intersection** stayed open: `countMutationRuns` was widened to handle names bound anywhere in the tree, `findUncountableRunShapes` was not, and a caller combining both evaded shapes — deps type hoisted into a sibling `*.types.ts` (no local binding to iterate) plus `run()` destructured off the deps property (nothing to count) — escaped both. Separately, the one-step `const { run } = useEditorMutation({...})` destructure named by round 2 was still green inside any file already listed in `COMMITTED_CALLERS`, `pages/EditorPage.tsx` included: the import signal added for it converts a file only from _invisible_ to _listed_, and the refusal was keyed on handle names, which `useEditorMutation`'s capital M defeats. Both reproduced by executing the shipped helper bodies against real repository source. Fixed: the offender pass now takes the same tree-wide handle set the counter does, and the one-step destructure is refused handle-independently. Commits `536404ef` (I1), `822a0f22` (I2).
- **Status correction (round 3, S3 — the comment stripper the counting rests on could DELETE code):** the pinned ceiling on `stripCommentsFromTsSource` claimed a regex literal containing a quote could at worst leave a real comment unstripped, "a false RED … not a silent green" — which was the entire justification for accepting the ceiling over a tokenizer. False in its load-bearing half: the fake string runs to the next quote, the scanner resumes mid-expression, and a `/*` inside a glob literal opens a block comment that deletes every line down to the next real `*/`. Verified with `const re = /"/;` above `glob("**/*.ts")` — the handle binding, the `run()` call and the committed branch all vanish, so counts go **down** and the file drops silently out of caller discovery. Regex literals now pass through verbatim. Commit `88894836`.
- **Status note (round 3, S4/S10):** `COMMITTED_CALLERS` recorded one integer asserted against two independently-derived quantities, so a legitimate divergence (two `run()` calls sharing one handler; one `run()` needing two comparisons) could not be encoded at all and the only route back to green was to weaken a detector — the value is now `{ runs, committedBranches }` (`8334ca4d`). And the `*** STOP` block a third caller's author reads now points at the deferred-seam decision recorded above, which is the decision that trigger is _for_ (`d12a50a8`).
- **Status correction (2026-08-19 code review round 4, I2/I3 — the fourth consecutive round to find the guard green-passing a shape it was recorded as blocking):** two independent holes, both in the silent-green direction. (a) `stripCommentsFromTsSource`, the stripper every count rests on, was still **deleting executable code**. Its regex-literal lookbehind class `[=(,:[!&|?{};]` held no `>` and admitted no keyword, so `=> /…/` and `return /…/` — two of the most common regex positions in TypeScript — never entered the regex branch; they fell through to the string alternative, which has no newline exclusion, so a fake literal ran to the next quote anywhere below and everything between was deleted. The backtick alternative separately could not span a `${…}` holding a nested template. Round 3's fixture (`const re = /"/;`) sits in a position the class _did_ admit, which is why the round-3 fix looked complete. Reproduced by execution; latent, not live — no current source diverged. (b) `HANDLE_RE`'s deps-property alternative required the type name **immediately** after the colon, so a handle declared `Pick<UseEditorMutationReturn, "run">` bound nothing, called nothing the counter could see, and did not trip the import signal (such a file imports the _type_, never the lowercase hook) — all three discovery signals false, the file skipped, both numeric assertions comparing unchanged numbers. Both `Pick<>`-narrowing a hook return and non-`mutation` handle names are established local habit; they only have to meet once. Fixed: (a) comment stripping is now done by the TypeScript **parser** rather than a fourth hand-patched regex — whether `/` opens a regex literal is a parser decision, so a bare `ts.createScanner` token loop would reintroduce the round-3 bug; parsing and removing the comment ranges TypeScript reports gets regex literals, nested template substitutions and JSX text right by construction, and all 44 pre-existing self-tests pass unmodified. (b) `HANDLE_RE` matches a _containing_ type reference bounded by `;`, `,`, newline, brace and paren (the paren bound stops the hook's own `(args: UseEditorMutationArgs): UseEditorMutationReturn` signature binding a handle named `args` — a mis-capture loses a run count silently, unlike over-listing), the renaming destructure `const { mutation: mut } = deps` is followed, and the import signal now reads comment-stripped source like its two siblings and accepts the type name. Commits `0d97601b` (I2), `b09de943` (I3/S2).
- **Status caveat (round 4 — what four rounds of this say):** every round since the guard shipped has found a shape it was recorded as blocking. The pattern is not carelessness in any one round; it is that the guard is a **collection of regexes approximating a parser**, and each round patched the one shape it had just found. Round 4 replaced the largest of those approximations with the real parser. `HANDLE_RE`, `runCallPattern`, `delegationPattern` and `importPatternFor` remain regexes over stripped text, so the class of miss is reduced, not closed. The standing answer if a fifth round finds another one is not a fifth patch — it is the seam fix this finding's own `Status reason (why not fix the seam)` defers, which makes the failure structurally impossible and needs no detector at all.
- **Status date:** 2026-08-19 18:30 UTC
- **Status commits:** `1abe3c91` (the guard), `36169a38` + `46ff26fb` (review round 1: comment-stripped counting, handle-binding discovery), `02ba9344` (shared `runCallPattern`), `6b77d25b` + `ee92d5e3` (review round 2: uncountable-shape refusal, three-signal discovery), `3b3d67a5` (string-aware stripper the counting rests on), `536404ef` + `822a0f22` (round 3: tree-wide offender pass, handle-independent one-step refusal), `5647174c` + `88894836` + `8334ca4d` + `d12a50a8` (round 3: shared matchers, regex-literal-aware stripper, two-field surface map, STOP-block trigger), `dafa3dfc` + `34d7edb2` (round 3 fallout: useless-escape lint on the regex-literal branch, `noUncheckedIndexedAccess` at the new helper call sites — both edit this finding's deliverable and were omitted from this list until round 4, S6), `0d97601b` + `b09de943` (round 4: parser-based comment stripping, containing-reference handle discovery). `1abe3c91` alone is precisely the guard this entry records as not having held.
- **Status date convention (round 3, S9):** the two `Status date` fields written by `25e68244` read "2026-08-19 14:52 UTC" — local time (`+0200`) labelled UTC, and an hour _ahead_ of the commit that wrote them (13:47 UTC). The neighbouring F-16 entry at 10:07 UTC follows the real convention. `Status date` is one of two fields a future session greps to order a finding's history against the log, so the label must mean what it says: these are now genuine UTC.

### [F-08] `useEditorMutationMachine.busy` / `isBusy()` / `getState()` are unconsumed, and `busy` is documented as knowingly wrong

- **Category:** 31 (Dead code) _(re-typed from 7)_
- **Impact:** Medium
...
- **Evidence:** `packages/client/src/hooks/useEditorMutationMachine.ts:13-22` — the `busy` doc says _"Do NOT gate on `machine.busy`; read `mutation.isBusy()` until a future phase … closes that gap."_ `:85-95`, `:108-109` declare `isBusy`/`getState`; grep confirms zero production call sites for `machine.isBusy()` or `machine.getState()` — the only non-test hit is `useEditorMutationMachine.test.tsx:108`.
- **Found by:** Coupling & Dependencies
- **Status:** Fixed
...
- **Found by:** Coupling & Dependencies
- **Status:** Fixed
- **Status reason:** Both rules enabled. `import/no-extraneous-dependencies` is configured per workspace, with `packageDir: [".", "packages/<ws>"]` so root tooling counts but a package declared only in a _sibling_ workspace still errors — that cross-workspace precision is the point, since F-11 was exactly that drift. Production source may not import a devDependency; tests and local config may. `import/no-cycle` is on repo-wide, with the real tree re-measured at 0 cycles, confirming [S-02].
- **Status caveat — the finding's premise was wrong in a way that matters:** "the plugin is already parsing the tree" is true, but it could not _resolve_ the tree. `import/no-cycle` was structurally incapable of firing on TypeScript: without an `import/resolver`, `import/no-unresolved` errors on a perfectly valid extensionless relative TS import, so the cycle walk never starts. An initial probe that reported "0 cycles across the repo" was therefore a **false green**, and simply switching the rule on — the literal fix this finding asks for — would have installed a guard that guards nothing while reading as a clean bill of health. Two `settings` entries are required and _each_ fails silently alone: `import/resolver: { typescript: true }` and `import/parsers: { "@typescript-eslint/parser": [...] }` (with only the former, resolution succeeds but the plugin cannot parse the _imported_ `.ts` file to read its imports, so a real two-file cycle is still missed). Both established by fixture, not by documentation.
...
- **Category:** 13 (Inconsistent boundaries) _(re-typed from 6)_
- **Impact:** Medium
...
- **Impact:** Medium
- **Explanation:** Two directions of drift, both unguarded because only `import/first` is enabled ([F-09]) — declared-but-unused packages, and _phantom_ dependencies that resolve only through npm hoisting.
- **Evidence:**
...
- **Status caveat:** the phantom `zod` was **worse than this finding described**. Root `node_modules/zod` is **4.3.6**, hoisted there by `eslint-plugin-react-hooks` — so `validateUuidParam.ts`, the UUID trust boundary, was resolving a _dev tool's_ transitive zod **a full major ahead** of the `3.25.76` that `@smudge/shared`'s schemas run on. The finding framed this as fragile-but-working; it was in fact a cross-major split. The fix pins server to shared's range. Residual: server and shared now hold separate physical copies of the same 3.25.76, which is harmless here only because nothing in the repo does `instanceof ZodError` — `outtakes.routes.ts:25` records the deliberate choice of structural checks over the invariant `ZodError<T>` generic. A future `instanceof` against a shared-thrown Zod error would silently fail.
...
- **Category:** 20 (Weak error handling strategy) _(re-typed from 34)_
- **Impact:** Medium
...
- **Evidence:** `packages/client/src/hooks/useKeyboardShortcuts.ts:190-193`:
  ...
  ```
  `handleSelectChapterWithFlush` (`EditorPage.tsx:904-936`) returns early when `switchToView("editor")` is false — the busy latch (`:755-758`, info banner), the editor lock (`:767-769`, **deliberately no banner**), and flush-save failure (`:814-818`, save-failed banner). Commit `3b95a804` fixed the gating _inside_ the function and left this call site's announcement outside it. The sibling Alt+Up/Down reorder path is unaffected.
- **Found by:** Error Handling & Observability
- **Status:** Fixed
...
- **Category:** 32 (Missing test coverage for critical paths)
- **Impact:** Medium
- **Explanation:** The final decompression-bomb defense — an archive whose central directory _under-declares_ sizes, slipping past `checkDeclaredSizes` — is dead in the coverage report, so Smudge's own guard is verified only by inspection.
...
- **Status note (why the existing T-1 could never have covered it):** established by execution, not inspection. JSZip checks each entry's _actual_ decompressed length against the size the central directory declares and throws `"Bug : uncompressed data size mismatch"`. T-1 patches exactly that field, so JSZip fires first every time and T-1's assertion passes through the generic post-move wrapper at `:313` — which is why its own comment concedes it accepts "either outcome". **Per-entry under-declaration can therefore never reach our guard.** What does reach it is a central directory listing the same path _twice_ with different declared sizes (duplicate-entry / "zip confusion"): `readCentralDirectorySizes` sums the small decoy while the extraction loop iterates those same duplicated names and extracts the real large entry on both passes. Every individual entry decompresses to exactly its declared length, so JSZip is satisfied and only the cumulative budget notices. **Precision (S2, 2026-08-18):** the duplication that matters is Smudge's own `names` array, not JSZip's map. T-2's forged record carries smudge.db's `localHeaderOffset`, so JSZip registers it under the _local_ name `smudge.db` and overwrites the real entry — `zip.files` holds no duplicate at all. The guard fires because `names` (read from the central directory) lists `big.bin` twice while `declaredTotal` counted the decoy at smudge.db's size.
...
- **Found by:** Structure & Boundaries
- **Note:** Distinct from accepted F-4, which accepted the slice _structure_ — the slice decomposition is exactly what F-4's acceptance cites as the interface's justifying value.

- **Status:** Fixed
- **Status reason:** `listChapterContentByProject` and `listAllChapterContentByProject` moved from the `ImagesStore` slice to `ChaptersStore` in `project-store.types.ts`, with their implementations moved into the `// --- Chapters ---` section of `sqlite-project-store.ts`. Both delegate to `chaptersRepo`, so by the slice header's own rule — "each slice owns the data operations for one domain, so a new operation edits only that slice" — chapters is where they belong. A comment at the new location records _why_ they were ever in images (their first callers were the image reference scan and reaper) so the move does not read as arbitrary.
- **Status note (zero risk _for this move_, and the reason is worth recording):** neither moved method belongs to a slice any parameter is typed against, so no call site's type changed. Nearly every consumer imports the composed `ProjectStore` (which `extends` all eight slices and is the same intersection either way), and moving a member between slices is invisible to those. Full suite before and after: 164 files, 3390 tests, identical.
- **Status note correction (S1, 2026-08-19 code review — do not read the above as a general rule):** this note originally said "nothing in the codebase is typed against a narrow slice", citing a search for `: ImagesStore` / `: ChaptersStore` only. There is exactly one slice-typed parameter in the tree: `insertAutoSnapshotIfChanged(txStore: SnapshotsStore, …)` in `packages/server/src/snapshots/auto-snapshot.ts:37`. It does not affect this fix — neither moved method is a `SnapshotsStore` member — but the general phrasing was wrong, and wrong in a load-bearing place: that same file documents a concurrency hazard arising _from_ slice-typed parameters (`auto-snapshot.ts:27-34` — the root store satisfies `SnapshotsStore` structurally too, and Knex's sqlite pool is `{ max: 1 }`, so passing a root store from inside a transaction blocks for the 60s `acquireConnectionTimeout`). A future move that touches `SnapshotsStore` must check that call site.
...
- **Impact:** Low _(down from Medium)_
...
- **Evidence:** `packages/client/src/components/OuttakesPanel.tsx:204-215` — _"Neither producer re-checks the project after its await — the capture POST lives in EditorPage and the blank-note POST in handleCreate below"_ — forcing a defensive third guard in `applyServerRow`. Call sites: `OuttakesPanel.tsx:265` + `EditorPage.tsx:1070`; same shape at `Editor.tsx:354` + `ImageGallery.tsx:206`.
- **Found by:** Structure & Boundaries
...
- **Found by:** Structure & Boundaries
- **Downgrade rationale:** the prop count is the direct, _accepted_ consequence of F-1's rendering extraction (state deliberately stays in `EditorPage`), and the prop list is a monitored surface (`editorEntryPointSurface.test.ts`). The residual concrete issue is the duplicated banner markup.

- **Status:** Fixed (the residual; the prop count was never in scope)
- **Status reason:** The three editor banners now render through one `EditorBanner` component (`packages/client/src/components/EditorBanner.tsx`). The duplication was concrete: the lock banner's container class string was **byte-identical** to `ActionErrorBanner`'s, the info banner's differed only in its three colour tokens (`bg-`, `text-`, `border-`), and the two dismiss buttons differed only in a colour token and an `aria-label`. `ActionErrorBanner` was deleted rather than kept alongside — it had exactly one consumer (`EditorMainContent`), so there was no migration surface and leaving it would have been dead code.
- **Status reason (why tone is one axis and not three):** `tone` (`"error" | "info"`) drives palette, ARIA role, `aria-live`, and the dismiss control's accessible name together, because those covary in all three uses and are not independently meaningful. Passing `role` separately from tone is how a dismissible notice ends up announced as an assertive alert. Class strings are stored as whole literals rather than assembled from fragments, so Tailwind's scanner still sees them.
- **Status note (the prop count is deliberately untouched):** F-18's headline names a 71-prop pass-through, but the finding's own downgrade rationale accepts that count as the direct consequence of the F-1 rendering extraction and names the residual as the banner markup. Only the residual was fixed. The prop list remains the monitored surface in `editorEntryPointSurface.test.ts`.
- **Status note (the safety net was written first and survived unmodified):** the Safety Net phase found the info banner's `role="status"`/`aria-live="polite"` and its dismiss control (`STRINGS.a11y.dismissInfo`) asserted **nowhere** — that string was referenced exactly once in the whole client, at the component itself. Every existing test reached the notice through `getByText`, which a role flip or a dropped dismiss button survives. A test was written first (commit `89bc9361`) pinning two distinct regions, distinct roles, each owning its own control, and one dismiss not clearing the other; it was confirmed red twice by negative control before this change. It then **passed unmodified** across the refactor, which is what it was written for. No test _assertion_ was touched by this fix — the only test-tree edit in `fc8cea1d` is a one-line comment in `e2e/chapter-create-recovery.spec.ts` renaming the deleted component.
- **Status follow-up (2026-08-19 code review, S1):** consolidation moved the lock banner's non-dismissibility from _structural_ (a dismiss control meant writing a button) to _a single omitted `onDismiss` prop_, identical in shape to the two sibling call sites two lines below it — and `STRINGS.a11y.dismissError` was asserted nowhere in `packages/client/src` or `e2e/`. The safety net asserted the lock region _contains_ Refresh but never that it contains no dismiss control. Pinned in commit `61d1c12f`, verified red by adding `onDismiss` to the lock banner.
...
- **Status follow-up (2026-08-19 code review, S5):** the caveat below understated the cost of the optional `onDismiss`. The deleted `ActionErrorBanner` declared it **required**, so an action error the user could not clear did not compile; consolidation made it optional and put the legitimately-undismissible lock banner two lines above the action-error call site in an identical shape, making the omission a plausible mechanical edit. `EditorBanner`'s props are now a union requiring exactly one of `onDismiss` / `children`, which restores that compile-time floor without outlawing the lock banner, and `EditorBanner.test.tsx` covers all three call shapes. Commit `71a8a5e5`, verified by negative control (removing `onDismiss` from the action-error call site is TS2322).
- **Status follow-up (2026-08-19 code review round 3, S1):** the union as first shipped typed `children` as `ReactNode`, which admits `null` / `undefined` / `false` — so it enforced prop _presence_, not _control_ presence: `<EditorBanner tone="error" message="x">{cond && <button/>}</EditorBanner>` type-checked and rendered an assertive `role="alert"` with no control whenever `cond` was false, the exact state the union exists to outlaw. No live call site did this; the plausible future edit is a conditional Refresh at `EditorMainContent.tsx`. Narrowed to `ReactElement`, with a companion `@ts-expect-error` case in `EditorBanner.test.tsx` passing a `false`-able child.
- **Status date:** 2026-08-19 16:56 UTC
- **Status commits:** `fc8cea1d` (the consolidation), `61d1c12f` (S1: lock banner pinned non-dismissible), `71a8a5e5` (S5: exactly-one-of prop union), `04e9f850` (round-3 S1: `children` narrowed to `ReactElement`). The Safety Net commit `89bc9361` precedes all four.
...
- **Status note (the finding understated the drift):** two further stale claims were found that the report did not catch, both inside §Accepted Architectural Trade-offs — the block that exists specifically so future reviews treat those decisions as settled. F-3's "13 call sites" was actually 45 and F-4's "51 one-line pass-throughs" was 61. A wrong number inside that block is worse than an incomplete module map, because it is load-bearing in an argument for _not_ fixing something. Both are now non-numeric, and the F-3 wording turns the growth into support for the locator rather than leaving a number to rot.
...
- **Found by:** Coupling & Dependencies
- **Note:** Distinct from accepted F-3, which describes the _locator_ and its "init once" contract, not the divergent re-init semantics between the two singletons it depends on. Downgraded to Low because production has exactly one `initDb` + one `initProjectStore` call (`index.ts:31,33`); the hazard is confined to test harnesses, which use `setDb`/`setProjectStore` instead.
- **Status:** Fixed
- **Status reason:** `initDb()` now throws `"Database already initialized — call closeDb() first"` when a handle already exists, mirroring `initProjectStore()`'s contract. That makes the hazard unreachable rather than merely documented: the store-over-destroyed-connection state required a *silent* second `initDb()`, and there is no longer one. `setDb()` is unchanged and remains the seam that deliberately replaces (used by `setupTestDb`). The reasoning is recorded at the function.
- **Status note (scope — the second asymmetry is accepted F-3, not this finding):** F-21 also observes that `closeDb()` does not reset the store and `resetProjectStore()` does not close the DB, so `index.ts:84-85` pairs them by hand. That was deliberately **not** changed. It is the accepted F-3 init-order contract restated — CLAUDE.md accepts the locator "with an init-order contract" as the price of a one-liner at every call site — and this finding's own Note scopes its novelty away from it: _"Distinct from accepted F-3, which describes the locator and its 'init once' contract, **not the divergent re-init semantics**."_ The divergent re-init semantics are what this fix closes. The alternative considered was having `initDb`/`closeDb` own the store lifecycle, which closes both halves but makes `db/connection.ts` import `stores/project-store.injectable` — a dependency edge from the database layer to the store layer, installed to solve a problem that exists only at process startup and shutdown. Rejected on that ground, not on cost.
...
- **Category:** 13 (Inconsistent boundaries)
- **Impact:** Low _(down from Medium)_
- **Explanation:** For chapters, the flag pairs with `content: null`, is internal, and is stripped at the wire boundary. For outtakes, the same field name pairs with a _valid empty doc_ and is part of the public wire type. A helper written against one convention is wrong for the other.
...
- **Status reason:** The server-authored `message` is gone from the duplicate 200 body and from the response type in `api/client.ts`, bringing this endpoint in line with the steering file's rule for the sibling success contracts — _the client owns the toast, the server ships no success copy_. The client needed no behavioural change: it already rendered `STRINGS.snapshots.duplicateSkipped` and never read the field.
...
- **Evidence:** `packages/server/src/snapshots/snapshots.service.ts:79-84` (`listSnapshots`), `:86-96` (`getSnapshot`), `packages/server/src/images/images.service.ts:100-105` (`listImages`). The fix and its reason live at `outtakes.service.ts:54-65`: _"S8: the liveness check and the read are ONE transaction… Split across two round trips, a project soft-delete landing between them answered 200-with-data where this file's own header says 404."_ `deleteSnapshot` (`:98-113`) also wraps both.
- **Found by:** Integration & Data
- **Status:** Skipped
...
- **Counter-argument, kept for honesty:** from the writer's view the title _field_ is the invalid input and is fixed in place, which is a defensible 400; the client routes on `error.code`, so nothing is user-visibly wrong today. Kept because it is a deviation from the project's own written rule and cheap to state.
- **Status:** Fixed
- **Status reason:** `ProjectTitleExistsError` now extends `ConflictError` (409) instead of `BadRequestError` (400). The code string `PROJECT_TITLE_EXISTS`, the message, and both throw sites (`createProject`, `updateProject`) are unchanged, so the only thing that moved is the status. The reasoning is recorded at the class definition, naming the two sibling conflicts it now matches (`IMAGE_IN_USE`, `RESTORE_CONFLICT`).
- **Status note (the client needed no production change, verified rather than assumed):** `mapApiError` resolves `byCode` before `byStatus` (`apiErrorMapper.ts:129`, pinned by the comment at `:112`), and both scopes carrying this code — `project.create` and `project.updateTitle` — map it by code, so the writer-visible copy is identical at either status. Neither scope declares `terminalStatuses`, so the retry classification does not move either. Both checked in the source, not inferred from the finding.
...
- **Category:** 34 (Inconsistent error/logging conventions)
- **Impact:** Low _(down from Low-Medium)_
...
- **Impact:** Low
- **Explanation:** The _owners_ are clean ([S-19]), but discoverability is not: there is no `.env.example`, no `docs/configuration.md`, and no Configuration section in `CLAUDE.md` or `CONTRIBUTING.md`.
...
- **Status:** Fixed
- **Status reason:** Added `docs/configuration.md` — one row per variable with its default, valid values, and the fail-fast-vs-warn-and-fallback distinction that matters when a value is wrong (`SMUDGE_PORT` refuses to start; `LOG_LEVEL` degrades quietly to `info` with only a stderr warning). Because the finding is a _re_-confirmation — the gap closed once and drifted back — it is guarded rather than merely written: `scripts/__tests__/configuration-doc.test.mjs` compares literal `process.env.NAME` reads in production source against the table in both directions. Confirmed red first (ENOENT on the missing doc), with a scanner self-test so a regex matching nothing cannot make it vacuous. A pointer was added to `CLAUDE.md` §Build & Run, closing the "no Configuration section" half of the finding.
...
- **Status caveat:** the guard is weaker than F-19's and says so in its own header. Its source of truth is a regex over source, not a directory listing, so it cannot see destructured (`const { X } = process.env`) or computed (`process.env[name]`) reads. Green means "no _literal_ `process.env.NAME` read is undocumented" — not "every environment variable is documented." Both the test header and the doc footer state this, so a future reader cannot mistake the green run for completeness.
...
- **Status:** Not yet fixed _(investigated 2026-08-19; kept open deliberately)_
