...
- **Status note (zero risk, and the reason is worth recording):** nothing in the codebase is typed against a narrow slice — a repo-wide search for `: ImagesStore` / `: ChaptersStore` parameters returns nothing. The slices exist solely to compose `ProjectStore`, which every consumer imports and which `extends` all eight. Moving a member between slices is therefore invisible to every call site and to the concrete class; `ProjectStore` is the same intersection either way. Full suite before and after: 164 files, 3390 tests, identical.
