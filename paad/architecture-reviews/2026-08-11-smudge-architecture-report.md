...
- **Status:** Fixed
- **Status reason:** Closed with a forcing-pause test rather than a change to the seam: `packages/client/src/__tests__/mutationCommittedSurface.test.ts` discovers every `mutation.run()` caller under `packages/client/src` and pairs each file's call count against its `committed_but_unreloaded` branch count. A third caller landing without a handler turns the surface assertion red; an existing caller losing a handler turns the pairing assertion red. Both were verified by negative control against the real source, and each fired on the *correct* assertion (a synthetic caller added to `EditorPage.tsx` → surface; one of `useFindReplaceController`'s two branches renamed → pairing, "expected 1 to be 2"). Comment lines are excluded, so the `useEditorMutation.run()` spelling used in prose throughout the editor hooks does not create phantom callers; four parser self-tests pin that, on the `editorEntryPointSurface.test.ts` precedent, so a silently-matching-nothing regex cannot make the check vacuous.
