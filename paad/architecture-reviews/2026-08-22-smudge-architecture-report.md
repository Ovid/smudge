...
- **Found by:** Integration & Data
- **Status:** Fixed
- **Status reason:** `getTrash` now resolves the LIVE project only, via `findProjectBySlug`. Because `getTrash` was its sole caller, the unfiltered `findBySlugIncludingDeleted` was removed from the repository, the `ProjectStore` slice interface, the `SqliteProjectStore` delegation and the method-surface guard — so the ambiguous lookup no longer exists for a future caller to rediscover. The sibling `findProjectByIdIncludingDeleted` / `updateProjectIncludingDeleted` stay: `restoreChapter`'s parent-restore branch genuinely needs them. Behaviour change: a soft-deleted project's slug now yields 404 on `/trash`, which is unreachable from the client — the trash view is only reachable for a project already loaded through the live-only `GET /api/projects/:slug`. Reproduced red first: the pre-fix endpoint returned the *soft-deleted* project's chapter id rather than the live project's own. **S8 (code review 2026-08-22): this retires a behaviour an earlier review explicitly asked for, and does so deliberately.** `paad/code-reviews/ovid-data-model-2026-04-08-21-03-44-456af01.md:31` says "Use `findBySlugIncludingDeleted` (or create one) for the trash endpoint, since viewing a deleted project's trash is a valid use case." That use case was specified and never built — nothing in the client can reach `/trash` for a project the live-only `GET /api/projects/:slug` cannot load — so there is no live regression, but a future session reading the older review would re-add the symbol without knowing it had been removed on purpose. If viewing a deleted project's trash is ever wanted, it needs a lookup that disambiguates by id, not a slug lookup that ignores `deleted_at`: the whole finding is that a slug is not a unique key across deleted rows. The symbol also survives in `docs/plans/2026-04-12-storage-architecture-plan.md:568` as part of the documented `ProjectStore` interface; that document describes a design at its date and is not being rewritten.
- **Status date:** 2026-08-22 15:29 UTC
- **Status commit:** 81a87fd9
...
- **Status reason:** Both halves closed. (1) `index.ts` binds the host returned by `getBindHost()` in the new `packages/server/src/config/loopback.ts`, which is `127.0.0.1`. No environment variable is read — Phase 7g.1 owns `SMUDGE_BIND_ADDRESS`, and its planned `0.0.0.0` default is the state this finding exists to remove, so the unsafe value is not reachable by configuration until that phase revisits it deliberately (7g.1 amended in the same commit). The wrapper exists so the value is decidable in a unit test and so 7g.1 has one seam. (2) `createApp()` rejects any request whose `Host` is not a loopback name, with 400 + `INVALID_HOST` (403 is not on the error-status allowlist; a new condition takes an existing status plus a discriminating code). **No `Origin` check was added, deliberately**: in a DNS-rebinding attack the page is same-origin with the target from the browser's point of view, so a GET carries no `Origin` header at all and only `Host` names the attacker's domain — an `Origin` allowlist would inspect a header the attack does not send, while needing four-plus entries covering dev/production/e2e in both `localhost` and `127.0.0.1` spellings. Authentication remains absent and is out of scope: it is a feature, which the fix-session PR-scope rule excludes. Verified against the real transport — `make e2e` 58/58, unchanged from baseline, because Vite proxies `/api` with `changeOrigin: true` so the server sees the proxy target's host.
  ...
  **Amended by the 2026-08-22 18:29 code review** (rule 6, `docs/roadmap-decisions/2026-08-19-architecture-fix-session-pr-scope.md`) — four follow-ups changed this finding's shipped deliverable:

  - **I6.** `isLoopbackHost`'s bracketed-IPv6 arm truncated at `]` and discarded what followed instead of requiring end-of-string or `:port`, so `[::1]evil.com` was accepted; the IPv4 arm range-checked no octet, so `127.999.999.999` was accepted. Both arms are now anchored by one regex and the IPv4 arm delegates to `node:net`'s `isIP`. Latent rather than live — the WHATWG URL parser rejects both spellings so no browser can produce one — and live the day 7g.1 widens the bind. The same commit accepts `localhost.`, the trailing-root-dot spelling a browser sends verbatim for `http://localhost./`, which used to 400 the whole app.
  - **I7.** The rejection was completely silent: a 400 `AppError` is below `globalErrorHandler`'s `status >= 500` log threshold, and `requestContext`'s access log is `debug`, suppressed at the default `info` level — so `app.ts`'s "Runs after `requestContext` so a rejection is traceable" was false as written. The middleware now emits `req.log.warn` with the rejected `Host`, truncated to 128 characters because it is attacker-controlled.
  - **S6.** Nothing tied `getBindHost()` to `isLoopbackHost`'s allowlist even though this doc block and roadmap 7g.1 both assert they must move together; `loopback.test.ts` now asserts `isLoopbackHost(getBindHost())`.
  - **OOSA2.** `DEFAULT_BIND_HOST` is no longer exported. Its only consumer was `expect(getBindHost()).toBe(DEFAULT_BIND_HOST)`, which cannot fail for any value of the constant.

  **Also corrected here: the "`make dev` and `make e2e` are unaffected" claim was argued only on the `Host` axis** (S9). The *bind* axis went unaddressed: the server now binds IPv4-only where it previously bound dual-stack, while `packages/client/vite.config.ts` and `playwright.config.ts` both target the *name* `localhost`. That survives only because Node >= 20 defaults `autoSelectFamily` on, which is a runtime default and not a property of this code. Two comments went stale with it and are fixed in the same follow-up: `Makefile:178`'s "the server's `app.listen(PORT)` defaults to `::` on dual-stack Linux", and CLAUDE.md's `make e2e-clean` two-probe rationale, whose `::1` probe can no longer see a live e2e server (the IPv4 probe still can, so the guard itself holds).

  **And S3:** `b66c3f77`, titled `test(architecture)`, also edited `config/loopback.ts` — `split(":")[0]` became `replace(/:.*$/, "")`, four commits after this block was written. The two spellings are behaviourally equivalent, verified by execution, so nothing shipped changed; the cost was traceability only, and it is recorded rather than amended because `b66c3f77` names the code-review report filed against it.
- **Status date:** 2026-08-22 15:39 UTC
- **Status commit:** 3d6a5bbc
