...
## `9e6f64b0` — `runRestore` validates central-directory names but extracts by JSZip's local-header names
- **File (at first sighting):** `packages/server/src/backup/backup-core.ts:285`
- **Symbol:** `runRestore`
- **Bug class:** Error Handling
- **Description:** `runRestore` builds `names` from `readCentralDirectorySizes` (CENTRAL directory) and gates on `names.includes("smudge.db")` at `:196`, but extraction does `zip.file(name)` and JSZip keys its map by LOCAL-header names (`zipEntry.js` `readLocalPart:82-85` overwrites `fileName`; `readCentralPart` deliberately skips it; `load.js:59-77` keys `zip.files` on `fileNameStr`). Overwriting 9 bytes of smudge.db's local-header name in a valid backup passes the presence check, hits `if (!file) continue` at `:285`, and `runRestore` RESOLVES SUCCESSFULLY having written nothing — after the live data dir was already moved aside. Reproduced end-to-end by two specialists independently. The same `continue` also lets a tampered archive silently drop arbitrary images. Original data survives at `movedAsideTo`, so this is a false success report rather than data loss.
- **Suggested fix:** Do NOT blanket-throw on `!file`. Verified: a real `runBackup` archive's central directory contains directory entries (JSZip auto-creates `images/` and each per-project folder) and `zip.file()` returns null for every one of them, which is why the skip exists — a blanket throw breaks every restore. Distinguish the two cases, e.g. `if (!file) { if (name.endsWith("/")) continue; throw new Error("archive entry declared in the central directory but not extractable: " + name); }`. Keep it inside the existing `try` so the `RestorePartialError` wrapper attaches the move-aside path. Closes the smudge.db and dropped-image variants at one site.
- **Confidence:** High
- **Found by:** Error Handling & Edge Cases, Security (`claude-opus-5[1m]`)
- **First seen:** 2026-08-18 on branch `ovid/architecture` at `0df45ecc`
- **Last seen:** 2026-08-18 on branch `ovid/architecture` at `0df45ecc`
- **Severity:** Important

## `4dcd11e8` — `Editor` never nulls `editorRef.current` on unmount, so `safeSetEditable` reports success against a destroyed editor
- **File (at first sighting):** `packages/client/src/components/Editor.tsx:441`
- **Symbol:** `Editor`
- **Bug class:** Logic
- **Description:** The effect publishing the `EditorHandle` (`Editor.tsx:440-497`, deps `[editor, editorRef]`) has NO cleanup, and `editorRef.current = null` appears nowhere in `packages/client/src` — unlike its sibling three lines above (`:362-364`), which nulls `editorInstanceRef` for exactly this stale-instance reason. After an unmount the ref holds a stale handle whose `setEditable` no-ops at `:493` (`if (editor && !editor.isDestroyed)`), so `safeSetEditable` (`editorSafeOps.ts:38-42`) applies nothing and returns TRUE — inverting its own documented contract ("true if it actually applied to a live editor"). This is the real mechanism behind report finding F-36, whose stated follow-up (making the false return observable) therefore cannot catch the class. `flushSave`, `markClean` and `insertImage` reach through the same stale handle.
- **Suggested fix:** Give the handle effect a cleanup: `return () => { if (editorRef) editorRef.current = null; };`. Register it AFTER the unmount-save effect so the S9 cleanup-ordering contract documented at `Editor.tsx:352-361` is preserved (React runs cleanups in registration order; the unmount-save closure must still see a live ref). Then correct the three comments that describe the old mechanism: `Editor.tsx:61-62`, `editorSafeOps.ts:17-19`, and the F-36 "deeper cause is untouched" caveat in `paad/architecture-reviews/2026-08-11-smudge-architecture-report.md`.
- **Confidence:** High
...
## `bbd22753` — `useSnapshotController` restore `committed_but_unreloaded` arm locks without a stale-chapter check
- **File (at first sighting):** `packages/client/src/hooks/useSnapshotController.ts:266-271`
- **Symbol:** `handleRestoreSnapshot`
- **Bug class:** Concurrency
- **Description:** The `result.stage === "committed_but_unreloaded"` arm calls `applyReloadFailedLock(STRINGS.snapshots.restoreSucceededReloadFailed)` unconditionally. Both siblings guard first: `useFindReplaceController.ts:129-131` computes `stale` from `targetChapterId` vs `getActiveChapter()?.id` and falls through to a dismissible action error, and this same file's 2xx-`BAD_JSON` arm at `:325-335` compares `getActiveChapter()?.id` against the restore target and substitutes chapter-attributed copy. If the active chapter drifted during the restore round trip, the persistent non-dismissible banner pins to, and disables, a chapter the restore never touched. Low reachability: drift normally settles as `MUTATION_SETTLED_SUPERSEDED` rather than `committed_but_unreloaded`.
- **Suggested fix:** Mirror the `:325-335` check in the `committed_but_unreloaded` arm: compare `getActiveChapter()?.id` with the restored chapter id and, on mismatch, surface the chapter-attributed dismissible action error instead of `applyReloadFailedLock`. Sibling-asymmetry cleanup, not urgent — the three paths should agree so a future reader does not have to re-derive which of them deliberately omits the check.
- **Confidence:** Medium
