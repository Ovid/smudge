...
## `1079f80c` — `$`-sequences in an image caption splice the document into the export
- **File (at first sighting):** `packages/server/src/export/image-resolver.ts:161-163`
- **Symbol:** `resolveImageSrcs` (figure/figcaption pass)
- **Bug class:** Error Handling
- **Description:** The figure/figcaption pass uses a *string* replacement whose replacement text interpolates `escapeHtml(fullCaption)`. `escapeHtml` (`export/html-escape.ts`) escapes only `& < > " '` — it does not neutralise `$`, and `$$`, `` $` ``, `$'`, `$&` are all special in a `String.replace` replacement string. `escapeHtml` also manufactures the hazard: it rewrites `'` → `&#39;` and `&` → `&amp;`, so a caption containing `$'` or `$&` becomes `$&…`, the match-substitution token. `fullCaption` comes from the user-editable `caption`/`source`/`license` columns set via `PATCH /api/images/:id`. `` $` `` injects the entire preceding portion of the rendered HTML into the figcaption; with a base64-embedded image that duplicates the whole document body once per captioned image, silently. Affects HTML, Markdown, plaintext and EPUB (all route through `resolveImageSrcs`); DOCX unaffected. Pre-existing on `main` — the adjacent C1 rewrite at `:152` converted the *sibling* substitution to a function replacer for regex-duplication reasons, leaving this one on the string form.
- **Suggested fix:** Use a function replacer, the same shape as the sibling eight lines above: `resolvedHtml.replace(re, (_m, tag: string) => `<figure>${tag}<figcaption>${escapeHtml(fullCaption)}</figcaption></figure>`)`. Function replacers use their return value verbatim — no `$` interpretation.
- **Confidence:** High
...
- **File (at first sighting):** `packages/client/src/components/ResizeSeparator.tsx:82-99`
- **Symbol:** `ResizeSeparator` (`onMouseDown` / `onMouseMove` / `cleanupResize`)
- **Bug class:** Concurrency
- **Description:** Press on the separator, drag the pointer outside the browser window, release there — the document never receives `mouseup`, so `cleanupResize` never runs and the `mousemove` listener stays attached; the panel then follows the pointer with no button held. Pressing the separator again installs drag 2 and overwrites `resizeCleanupRef.current` without invoking drag 1's cleanup, so both `mousemove` handlers fire per event with different `startWidth` baselines and the width oscillates; drag 1's listener is unreachable for the rest of the page's life. Pre-existing: `ReferencePanel` and `Sidebar` carried byte-identical copies before the Phase 4c.2 I7 extraction, which is a verbatim move. Recorded because the extraction gives the defect a single owner, making it a one-place fix instead of two.
- **Suggested fix:** In `onMouseMove`, bail when no button is held — `if (ev.buttons === 0) { cleanupResize(); return; }` — and call `resizeCleanupRef.current?.();` at the top of `onMouseDown` so a stale drag cannot coexist with a new one. Pointer capture (`setPointerCapture` + `pointerup`) is the fuller fix if document-level listeners are unwanted.
- **Confidence:** Medium
