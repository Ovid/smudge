...
## `04952dd1` — Test named for `applyImageRefDiff` coverage asserts nothing about it
- **File (at first sighting):** `packages/server/src/__tests__/search.service.test.ts:653`
- **Symbol:** `<file-scope>`
- **Bug class:** Logic
- **Description:** The test "image reference counts adjusted via applyImageRefDiff" replaces `hello` with `goodbye` in a doc whose only image node is untouched, so the ref diff is empty (`added=[]`, `removed=[]`) and `reference_count` reads 1 whether or not `applyImageRefDiff` runs. Confirmed by execution: deleting the `applyImageRefDiff` call from `packages/server/src/chapters/chapter-content-write.ts` fails exactly one test in the server suite — a *snapshots* test — with this one green among the other 1082. `replaceInDoc` preserves an inline image node through its run/separator re-weave, so the diff is always a no-op on the replace path, which is exactly what the call is a backstop against. Kept out of scope: before the F-03 extraction, deleting the inline call in `search.service.ts` would have failed zero tests, so consolidating two call sites behind one covered helper reduced the blast radius rather than widening it.
- **Suggested fix:** Either alter the chapter's image set on the replace path so the diff is non-empty, or spy on `incrementImageReferenceCount` and assert it is reached from `replaceInProject`. Renaming the test would also be honest if the coverage is not wanted.
- **Confidence:** Medium
- **Found by:** Logic & Correctness (`claude-opus-5[1m]`)
- **First seen:** 2026-08-21 on branch `ovid/architecture` at `1b96b3b4`
- **Last seen:** 2026-08-21 on branch `ovid/architecture` at `1b96b3b4`
- **Severity:** Suggestion
