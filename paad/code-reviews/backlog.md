...
- **FIXED 2026-08-23 by `0a1c6f3c`.** The mock now rejects with `ApiRequestError("[dev] Failed to fetch", 0, "NETWORK")` — what `apiFetch` actually produces — and asserts `saveFailedNetwork`. The file's `api/client` mock gained the `code` field the mapper keys on. Verified load-bearing: deleting `chapter.save`'s `network:` entry turns it red. **Wider sweep done:** the other nine raw-`TypeError` rejections are correct — the eight in `api-client.test.ts` mock `fetch` (or a response method) itself, the layer that does the wrapping, and `EditorPageFeatures.test.tsx`'s one is the deliberate I2 non-`ApiRequestError` case. (S9, review round 2: this said "four other". The two groups it describes do cover all nine sites; only the count was wrong.)
...
- **FIXED 2026-08-23 by `a254d743`, closed as part of `8b34a209` (marker added 2026-08-23, review S12).** The inside-updater identity re-test this entry asks for is on the recovery arm's `setProject`, and `12d20263` later added the snapshot-identity conjunct (`refreshed.id === projectId`) beside it. The entry was addressed but carried no close marker, against this file's own §Entry lifecycle rule — and it is cited both by live code (the `a65acf76` reference in `useChapterCrud.ts`) and by a `describe` block in `useChapterCrud.epochGuard.test.tsx`, so it is exactly the class of id that must resolve to something. Residual: see `9c2ad4e1`.
...
- **Residual documented in code, not fixed — enumeration corrected 2026-08-23 (review I6), and corrected again the same day (review round 2, S1).** SIX writes keep the outer guard only. Success path: `setActiveChapter`, `setChapterWordCount`, and the `confirmedStatusRef.current[newChapter.id]` seed after the `setProject` merge. Recovery path: `replaceConfirmedStatusesFromProject(refreshed)` plus its own `setActiveChapter`/`setChapterWordCount`. The count has been wrong twice in opposite directions, which is why it is now spelled out write by write. The ORIGINAL text said "the `confirmedStatusRef` seed", singular — it named the success-path single-key seed and dropped the recovery arm's WHOLE-MAP replacement, the more damaging of the two, since it evicts every live baseline rather than adding one. The I6 rewrite then said "FOUR" over a five-item list and dropped the success-path seed the original HAD named, swapping one omission for the opposite one. Per CLAUDE.md F-19 the enumeration is the mitigation, so the unnamed residual had none. Note also that the obvious patch does not work: adding a statement-time re-check before those writes, as `useTrashManager` does, cannot close the drain window, because `projectRef.current` is a render-body mirror and still reads the OLD project at that point. The additive-write fix that would close it is filed as `9c2ad4e1`.
...
- **FIXED 2026-08-23 by the review-response round on `ovid/backlog`.** Two guards, not one, and neither at the suggested line. (a) Before the active-chapter block — not right after the DELETE's abort check, because `clearCachedContent(chapter.id)` above it must still run on drift: the chapter really is deleted server-side, so its cached draft is garbage either way, and bailing earlier would leak that entry. (b) After the secondary GET resolves — the entry's single guard would have run *before* that GET was issued, while the navigation it defends against typically happens *during* it. Both return `true`, matching the two inner arms: the delete succeeded, so the caller should still refresh its trash list; only the UI writes are skipped. Regression test in `useProjectEditor.test.ts` parks the post-delete GET, navigates A→B, lets B's load finish, then answers the GET — red before the fix, with project A's `ch2` pinned as active under project B's URL.
- **Confidence:** Medium
...
- **FIXED 2026-08-23 by the review-response round on `ovid/backlog` — by DELETING the line numbers, not correcting them.** Renumbering would have re-armed the same trap for the next edit above any of those lines, and §Documentation Discipline rule 2, which sits in the same file, already says what to do instead: "Cite symbols, not line ranges … a symbol name survives the next edit above it." The paragraph the entry names carried **eleven** line citations, not the five it counted — `app.ts:41,45,46,50,52`, `images.routes.ts:39`, `images.paths.ts:94-96`, `outtakes.routes.ts:14,44`, `client.ts:605,619`, `projects.service.ts:155`, `useChapterMetadata.ts:103-118`, `useProjectTitleEditing.ts:29-33`, `useTrashManager.ts:194-207`, `2026-03-29-project-slugs-design.md:12`, and `dist/index.js:977`. All are now symbol names (`createApp`'s five mounts, `imagesRouter`'s `requireUuidParam("projectId")` guard, `projectOuttakesRouter`'s `/:id/outtakes` routes, `api.search.find` / `api.search.replace`, `updateProject`'s `resolveUniqueSlug` call, and the three client slug-holder handlers), each verified to exist by grep before substitution. One exception kept deliberately: `dist/index.js:977` is third-party build output under `node_modules` with no stable symbol to name, so it keeps its line number with the reason stated inline. The only other `file:line` left in CLAUDE.md is rule 2's own worked example of the failure, which must stay.
- **Confidence:** High
...
- **Disclosure: published deliberately (S8, review 2026-08-23 round 2).** This entry carries reproduction-level detail of an UNFIXED weakness in a git-tracked file on a public remote. That was weighed and kept. The precondition for the exploit is that the operator has already chosen to run `make restore` on an archive a stranger supplied, from a shell in a source checkout, against a single-user app bound to `127.0.0.1` that never fetches an archive on its own — and at that point the archive is replacing the database wholesale, so the bomb gate is not the interesting failure. Against that, the measured reproduction is the most valuable thing here: round 2's I2 exists precisely because the earlier, vaguer record would have led the next author to under-scope the single-parser work. **Revisit if the fix has not landed by the time roadmap Phase 7g.1 widens the network bind or Phase 8b makes backups a product feature rather than an operator script** — the threat model above is what makes the disclosure cheap, and both phases change it.
...
- **FIXED 2026-08-23 by the `[r2 OOSI1/OOSS1]` commit on `ovid/backlog`.** The merge arm now reads `if (!isStaleProject() && refreshed.id === projectId)`, mirroring the sibling guard in `useChapterCrud.handleCreateChapter`'s recovery arm (S14). Both the `setProject` and the session-permanent `projectSlugRef.current` write sit inside it, so the second axis this entry flagged is covered at the same site. Pinned by `useChapterMetadata.recoveryIdentity.test.tsx`, which arms the recovery GET to return a DIFFERENT project under the same slug — the case `isStaleProject()` reports as "not stale" by design.
- **Confidence:** Medium
...
- **FIXED 2026-08-23 by the `[r2 OOSI1/OOSS1]` commit on `ovid/backlog`.** `if (isStaleProject()) return undefined;` now sits between the `devWarn` and the 404 lock, matching the guard on the success arm three lines above. The `devWarn` stays unguarded, per this entry's own suggested fix. Pinned by two tests: one advances the refs to project B while the recovery GET is in flight and asserts no lock, one keeps the writer on A and asserts the lock still fires.
- **Confidence:** Medium
- **Found by:** Logic & Correctness (`claude-opus-5[1m]`)
- **First seen:** 2026-08-23 on branch `ovid/backlog` at `b74354de`
- **Last seen:** 2026-08-23 on branch `ovid/backlog` at `b74354de`
...
- **FIXED 2026-08-23 by the `[r2 OOSS2]` commit on `ovid/backlog`.** `switchToView` now builds `makeStaleProjectGuard(projectRef, projectSlugRef)` at entry, matching the nine sibling handlers, and bails before every post-await write: both failure arms' `setActionError` + `safeSetEditable(true)`, and the success arm's `setTrashOpen`/`setViewMode`/`setDashboardRefreshKey`. The `clientWarn` on the throw path stays unguarded — it is dev-facing and the throw really happened. Pinned by a test that performs a REAL route navigation between two slugs on the same `/projects/:slug` pattern, so React Router reuses the EditorPage instance exactly as production does; a keyed route would remount and the bug could not exist.
- **Note on the latch half of this entry.** The entry also observed that `viewSwitchInFlightRef` stays latched across the navigation. That ref no longer exists — the `[r2 I4]` commit replaced it with the shared `actionBusyRef`, which has the same bounded property: raised for the flush window, cleared in the same `finally` whichever project is on screen when it settles.
- **Confidence:** Medium
- **Found by:** Concurrency & State (`claude-opus-5[1m]`)
- **First seen:** 2026-08-23 on branch `ovid/backlog` at `b74354de`
- **Last seen:** 2026-08-23 on branch `ovid/backlog` at `b74354de`
...
- **FIXED 2026-08-23 by the `[r2 OOSI2]` commit on `ovid/backlog`.** `walkCentralDirectory` now reads CEN+20 alongside the CEN+24 it already read, and `runRestore` passes the summed per-entry compressed total as the denominator, clamped by `buf.length` so forged-LARGE compressed sizes cannot raise it instead. Pinned by three tests, the load-bearing one driving `runRestore` end to end — the two that exercise `checkDeclaredSizes` directly do not reach the call site where the defect actually lived.
- **Magnitude corrected while fixing.** The report described "a 100 KB bomb padded to ~200 MiB". Measured, the technique has a hard ceiling well below that: `findEocdOffset` scans backward at most 64 KiB, so padding past that puts the EOCD out of reach and `readCentralDirectorySizes` throws "not a valid zip (no EOCD)" at step 1 — refused, not mis-measured. Pinned by a third test so the larger claim does not come back. 64 KiB was still ample: a measured 27.9x archive diluted to 2.86x, comfortably under `maxRatio: 10`, which is why this was worth fixing rather than downgrading.
- **Confidence:** Medium
...
- **FIXED 2026-08-23 by the `[r2 OOSS3]` commit on `ovid/backlog`.** Not by a shared stub — by no stub. Vitest hands a mock factory `importOriginal`, so all nineteen factories now take the REAL class from the real module (`async (importOriginal) => ({ ApiRequestError: (await importOriginal<typeof import("../api/client")>()).ApiRequestError, api: { ... } })`). Drift is impossible by construction rather than centralised, which is strictly better than the suggested factory: a shared stub is still one hand-maintained copy of a contract, and `staleProjectGuard.ts`'s own header records what happens to those ("nine copies is how four of them drifted unnoticed"). `apiRequestErrorStubDrift.test.ts` is the forcing pause — it fails if any file under `packages/client/src` other than `api/client.ts` declares the class, and it pins the real four-parameter arity the stubs were supposed to match.
- **Impact claim corrected while fixing.** The entry said "every error those suites construct maps to the scope FALLBACK ... deleting one of them would leave those suites green". Checked against the tree: `App.test.tsx`, `KeyboardShortcuts.test.tsx` and `StatusBar.test.tsx` construct **zero** `ApiRequestError`s — their stubs existed only to satisfy the module factory. Every file that DID construct errors already carried `code`. So no suite was silently green; the defect was latent (the next test written in one of those three files would have inherited a broken stub with no signal) plus one live gap: no stub carried `extras`, so scope `extras` was untestable through any of them.
- **Confidence:** Medium
- **Found by:** Contract & Integration (`claude-opus-5[1m]`)
- **First seen:** 2026-08-23 on branch `ovid/backlog` at `b74354de`
- **Last seen:** 2026-08-23 on branch `ovid/backlog` at `b74354de`
