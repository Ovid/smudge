...
- **Important differences:** Two sites in `search.routes.ts` (`:94–97` and `:146–149`) emit `NOT_FOUND` after a service call returns `null` rather than after a direct slug lookup — verified as TOCTOU re-checks for "project not found" (the slug resolved at `:84–90` / `:128–135` but the project went away between then and the service returning); the envelope shape and message string match the slug-lookup not-found, so they are not a behavioral exception. No site adds logging, headers, or extras. The resource-name string is the only varying field across all 20 sites.
