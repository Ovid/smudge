...
export function EditorPage() {
  ...
  // F-04: outtake capture (the toolbar's copy-selection-to-drawer flow) lives
  // in its own controller hook, alongside the snapshot and find-replace seams.
  // It takes no mutation/lock handles on purpose — capture never writes editor
  // content. The hook's header explains why its destructive sibling (roadmap
  // 4c.2a) will not inherit that exemption.
  const { capturedOuttake, outtakesExternalRefreshKey, handleSendSelectionToOuttakes } =
    useOuttakeCapture({
      toolbarEditor,
      project,
      activeChapter,
