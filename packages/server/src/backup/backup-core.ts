...
import {
  ...
  validateEntryPaths,
  type BombLimits,
} from "./backup-zip-format";

// F-17: the ZIP wire-format parsing + zip-slip/bomb primitives moved to
// backup-zip-format.ts (own module, imported by both production here and the
// bomb/zip-slip tests so the byte-offset logic stays single-owned — S9). Re-
// exported so existing importers (`../backup-core`) are unchanged.
export {
  DEFAULT_BOMB_LIMITS,
  ZipSlipError,
  DecompressionBombError,
...
export type { CentralDirEntry, BombLimits } from "./backup-zip-format";
