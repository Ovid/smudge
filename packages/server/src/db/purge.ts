...
export async function purgeOldTrash(db: Knex): Promise<{ chapters: number; projects: number }> {
  ...
  let chapters = await db("chapters").where("deleted_at", "<", cutoff).delete();

  // Find projects to purge
  const projectsToPurge = await db("projects").where("deleted_at", "<", cutoff).select("id");
  ...
  const projects = await db("projects").where("deleted_at", "<", cutoff).delete();
